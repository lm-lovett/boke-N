...
> Python Workers 需要 `python_workers` compatibility flag；`wrangler.jsonc` 中 `disable_python_external_sdk: true` 表示使用 pywrangler 打包的 vendored `workers` 模块，而非运行时外部 SDK。
